...
<body>
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        echo "<table border='1'>";
        echo "<tr><th>Roll_Number</th><th>Full_Name</th><th>DBATU_Enrollment_Number</th><th>Current_Year</th><th>Email_Address</th><th>Personal_Mobile_Number</th></tr>";
        // Loop through each row in the result set
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>";
            echo "<td>" . htmlspecialchars($row['Roll_Number']) . "</td>";
            echo "<td><a href='detail.php?id={$row['id']}'>{$row['Full_Name']}</a></td>";
            echo "<td>" . htmlspecialchars($row['DBATU_Enrollment_Number']) . "</td>";
            echo "<td>" . htmlspecialchars($row['Current_Year']) . "</td>";
            echo "<td>" . htmlspecialchars($row['Email_Address']) . "</td>";
            echo "<td>" . htmlspecialchars($row['Personal_Mobile_Number']) . "</td>";
            echo "</tr>";
        }
